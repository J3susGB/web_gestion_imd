...
<script>
    function enviarFormularioManualmente() {
        ...
        setTimeout(() => {
            window.location.href = '/admin/partidos?' + params.toString();
...
<script>
    function restablecerFiltros(event) {
        ...
        window.location.href = '/admin/partidos'; // Recarga la página limpia
    }
</script>
                    ...
                    <div id="<?php echo $p->id_partido; ?>" class="partidos__partido" data-partido-id="<?php echo $p->id_partido; ?>">
                        <div class="partidos__partido--id">
                            ...
                            <div class="partidos__partido--logos">
                                <p>
                                    <?php echo $p->id_partido . " - " . $p->categoria; ?>
                                </p>
                                <?php if ($p->id_usuario) {
                                    // Buscar el perfil correspondiente
                                    $observacion = '';
                                    foreach ($perfiles as $perfi) {
                                        if ($p->id_usuario == $perfi->id) {
                                            $observacion = 'Última modificación: ' . htmlspecialchars($perfi->usuario, ENT_QUOTES, 'UTF-8');
                                            break; // Salir del bucle una vez encontrado
                                        }
                                    }
                                ?>
                                    <picture>
                                        <source srcset="/build/img/modi.avif" type="image/avif">
                                        <source srcset="/build/img/modi.webp" type="image/webp">
                                        <img class="modi" id="modi" loading="lazy" width="20" height="20" src="/build/img/modi.png" alt="botón para resetear"
                                            data-observaciones="<?php echo $observacion; ?>">
                                    </picture>
                                <?php } ?>
                            </div>
